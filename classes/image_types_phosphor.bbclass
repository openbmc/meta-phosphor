...
IMAGE_TYPEDEP_mmc-verity = "${IMAGE_BASETYPE}"
...
python do_generate_mmc_verity() {
    ...
    def align_up(number, boundary):
        return int((number + (boundary - 1)) / boundary) * boundary

    # verity metadata must be aligned on the block boundary subsequent to the
    # end of the data
    verity_hash_offset = align_up(rootfs_image_size, sector_size)

    verity_hash_blocks = int(verity_hash_offset / block_size)
    ...
    a_sectors = align_up(verity_image_size, block_size) / sector_size
    a_linear="a,,,rw, 0 %i linear /dev/mmcblk0 0" % a_sectors
    root_sectors = align_up(rootfs_image_size, block_size) / sector_size
    root_verity="root,,,ro, 0 %i verity 1 /dev/dm-0 /dev/dm-0 %i %i %i %i %s %s %s" % (
                    root_sectors,
                    block_size,
                    block_size,
                    verity_hash_blocks,
                    verity_hash_blocks + 1,
                    verity_algo,
                    verity_root,
                    verity_salt)
    ...
    mmc_image_path = os.path.join(d.getVar('IMGDEPLOYDIR', True),
                                    '%s' % mmc_image_name)
    csd_size = (1 << (9 + 9 - 1 + 2))
    subprocess.check_call(['fallocate',
                           '-l',
                           '%i' % align_up(verity_image_size, csd_size),
                           '%s' % mmc_image_path])
    subprocess.check_call(['dd',
                           'if=%s' % verity_image,
                           'of=%s' % mmc_image_path,
    ...
    subprocess.check_call(['ln', '-sf', mmc_image_name, mmc_link_name])
}
