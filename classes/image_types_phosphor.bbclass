...
# eMMC sizes for the rootfs content. Assumes a minimum FLASH_SIZE of 12 GB.
# Rootfs A (1 GB)
...
do_mk_mmc_empty_ext4_images() {
    ...
    if [ ! -e ${other_rwfs} ]; then
        mk_empty_image ${other_rwfs} ${MMC_OTHER_FW_SIZE}
...
python do_generate_mmc_ext4() {
    ...
    start_offset = 0;
    end_offset = int(d.getVar('MMC_ROFS_A_SIZE', True))
    ...
    start_offset = end_offset;
    end_offset += int(d.getVar('MMC_ROFS_B_SIZE', True))
    ...
    start_offset = end_offset;
    end_offset += int(d.getVar('MMC_RWFS_SIZE', True))
    ...
    start_offset = end_offset;
    end_offset += int(d.getVar('FLASH_SIZE', True))
    ...
    def get_sectors(var_size_kb):
        size = int(d.getVar(var_size_kb, True))
        sector_size = 512
        return size * 1024 // sector_size

    # Store the sector size information for the initramfs to use with dmsetup
    env_file = os.path.join(d.getVar('IMGDEPLOYDIR', True),
                            '%s.dm.env' % d.getVar('IMAGE_NAME', True))
    with open(env_file, 'w') as fd:
        fd.write('ROFS_A_SECTORS={}\n'.format(get_sectors('MMC_ROFS_A_SIZE')))
        fd.write('ROFS_B_SECTORS={}\n'.format(get_sectors('MMC_ROFS_B_SIZE')))
        fd.write('RWFS_SECTORS={}\n'.format(get_sectors('MMC_RWFS_SIZE')))
        fd.write('OTHER_FW_SECTORS={}\n'.format(get_sectors('MMC_OTHER_FW_SIZE')))
