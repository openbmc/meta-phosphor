...
pkg_postinst_${PN}() {
	...
	for i in $CHASSIS_INSTANCES; do
		...
		# Link each instance of phosphor-regulators-monitor against obmc-chassis-poweron wants target
...
pkg_prerm_${PN}() {
	...
	for i in $CHASSIS_INSTANCES; do
		...
		rm "$D$systemd_system_unitdir/obmc-chassis-poweron@$i.target.wants/phosphor-regulators-monitor@$i.service"
	done
}
