...
# Set the following distro features to enable the different functions. Ex:
# DISTRO_FEATURES += "http-redirect-awk"
PACKAGECONFIG ??= " \
    ${@bb.utils.contains('DISTRO_FEATURES', 'first-boot-set-mac', 'first_boot_set_mac', '', d)} \
    ${@bb.utils.contains('DISTRO_FEATURES', 'first-boot-set-hostname', 'first_boot_set_hostname', '', d)} \
    ${@bb.utils.contains('DISTRO_FEATURES', 'http-redirect-awk', 'http_redirect', '', d)} \
    "
...
RDEPENDS_${PN} += " \
    ${@bb.utils.contains('PACKAGECONFIG', 'http_redirect', 'busybox', '', d)} \
    "
